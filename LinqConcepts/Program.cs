    ...
    public static void Main(string[] args)
    {
        ...
        switch(choice){
            ...
            case 3:{
                TraineeData trainee = new TraineeData();
                List<TraineeDetails> traineeList = trainee.GetTraineeDetails();
                IEnumerable<TraineeDetails> data = traineeList.Skip(traineeList.Count-2) ;
                for(int iterator = 0;iterator < data.Count(); iterator++){
                    System.Console.WriteLine($"{data.ElementAt(iterator).TraineeId}");
                }
                break;
            ...
            case 7:{
                TraineeData trainee = new TraineeData();
                List<TraineeDetails> traineeList = trainee.GetTraineeDetails();
                IEnumerable<TraineeDetails> trainees = from data in traineeList select data;
                foreach(TraineeDetails trainee1 in trainees){
            ...
            case 9:{
                ...
                IEnumerable<TraineeDetails> trainees = from data in traineeDataList where data.TraineeId == traineeid select data;
                foreach(TraineeDetails trainee in trainees){
            ...
            case 12:{
                    TraineeData obj = new TraineeData();
                    List<TraineeDetails> traineeDataList = obj.GetTraineeDetails();
                    IEnumerable<TraineeDetails> trainees = traineeDataList.Where(s=>true);
                    foreach(TraineeDetails trainee in trainees){
            ...
            case 16:{
                ...
                List<int> sumList = new List<int>();
                List<TraineeDetails> traineeDataList = obj.GetTraineeDetails();
                IEnumerable<TraineeDetails> trainees = traineeDataList.Select(s=>s);
                foreach(TraineeDetails trainee in trainees){
                    IEnumerable<int> traineeScores = trainee.ScoreDetails.Select(s=>s.Mark);
                    int sum = traineeScores.Aggregate((a,b)=>a+b);
                    sumList.Add(sum);
                }
                bool result = sumList.Where(s => s > 40).Select(s => s).Any();
            ...
            case 17:{
                TraineeData obj = new TraineeData();
                List<int> sumList = new List<int>();
                IEnumerable<TraineeDetails> trainees = obj.GetTraineeDetails().Select(s=>s);
                foreach(TraineeDetails trainee in trainees){
                    IEnumerable<int> traineeScores = trainee.ScoreDetails.Select(s=>s.Mark);
                    int sum = traineeScores.Aggregate((a,b)=>a+b);
                    sumList.Add(sum);
            ...
            case 18:{
                ...
                IEnumerable<TraineeDetails> trainees = data.GetTraineeDetails().OrderByDescending(s=>s.TraineeName).Select(s=>s);
        ...
        */
        ...
        System.Console.WriteLine();
        TraineeData obj = new TraineeData();
        /*Dictionary<string,TraineeDetails> trainees = obj.GetTraineeDetails().OrderByDescending(s=>s.DOJ).ThenByDescending(s=>s.TraineeId).ToDictionary(s=>s.TraineeId,s=>s);
        foreach(KeyValuePair<string,TraineeDetails> trainee in trainees){
